...
Field::Field(int _width, int _height, int num, float fi, APPROX_FIELD _approx_type)
{
	...
	{
		float D = static_cast<float>(width) / static_cast<float>(num + 1); // рассто€ние между затравками
		float d_max = D; // рассто€ние от крайней затравки до кра€ области
		float d_min = 15; // параметр характеризующий размер иглы (вли€ет сильно, но не пон€тно как выбирать)
		lambda_max = 1;				// 
		lambda_min = height - 1;	// диапазон параметра л€мбда
